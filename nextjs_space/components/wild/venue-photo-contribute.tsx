...
export function VenuePhotoContribute({hubId}:{hubId:string}) {
 ...
 async function submit(e:React.FormEvent<HTMLFormElement>) {
  ...
  try {
   ...
   const r=await fetch(`/api/wild/journal/contribute/${hubId}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({base64:btoa(binary),caption:data.get('caption'),credit:data.get('credit'),location:data.get('location'),observedOn:data.get('observedOn'),adult:data.get('adult')==='on',rightsConfirmed:data.get('rights')==='on',venueUseConsent:data.get('permission')==='on',scannerConsent:data.get('scanner')==='on'})})
