...
function AccountWorkspace() {
  ...
  return <div className="space-y-6">
    <header><h1 className="font-display text-3xl font-bold tracking-tight">My workspace</h1><p className="mt-2 max-w-2xl text-muted-foreground">Continue your saved work below, or create a separate workspace for a new project.</p></header>
    ...
    <section className={panel} aria-labelledby="existing-heading">
      ...
      {loading ? <p className="mt-4" role="status">Loading your workspaces…</p> : workspaces.length === 0 ? <p className="mt-3 text-sm text-muted-foreground">No workspaces yet. Choose Create a workspace below, or open an invitation from a colleague.</p> : <ul className="mt-4 space-y-3">{workspaces.map(item => (
    ...
    <details><summary className="cursor-pointer rounded border p-4 font-medium">Create a workspace</summary>
    <section aria-labelledby="persona-heading" className="space-y-4">
      <div><h2 id="persona-heading" className="font-display text-xl font-semibold">Start a new workspace</h2><p className="mt-1 text-sm text-muted-foreground">Every choice opens the same universal canvas — upload PDF, TXT or CSV files (up to 3 MB each). The preset only sets your default map layers, export title and suggested prompts. You can change everything later.</p></div>
