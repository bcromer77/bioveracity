...
function AccountWorkspace() {
  ...
  return <div className="space-y-6">
    <header><h1 className="font-display text-3xl font-bold tracking-tight">My workspace</h1><p className="mt-2 max-w-2xl text-muted-foreground">Organise sources, review events and build a case you can trace back to the evidence.</p></header>
...
function CaseBoard({ workspaceId }: { workspaceId: string }) {
  ...
  return <div className="space-y-6">
    ...
    {selected && <CaseSummary key={selected} workspaceId={workspaceId} caseId={selected} onSelectCase={setSelected} />}
  </div>
}

function CaseSummary({ workspaceId, caseId, onSelectCase }: { workspaceId: string; caseId: string; onSelectCase:(id:string)=>void }) {
