...
export default async function VenuePage({params}:{params:Promise<{id:string}>}){
 ...
 <section className="bv-section"><p className="bv-eyebrow">Beyond the doorstep</p><h2>Discover the wider area.</h2><p className="bv-lead">A starting point for understanding the landscape. These are regional stories, not wildlife sightings at the venue or directions to publicly accessible sites.</p><div className="bv-grid bv-three">{venue.countyData.topics.map(topic=><article className="bv-feature bv-topic" key={topic.slug}><span className="bv-pill">{topic.evidenceScope==='national-context'?'Northern Ireland context':topic.evidenceScope==='county'?'County context':'Wider landscape'}</span><h3>{topic.title}</h3><p>{topic.summary}</p>{topic.caveat&&<p className="bv-small">{topic.caveat}</p>}<a href={topic.sourceUrl}>Read the source ↗</a><p className="bv-small">{topic.publisher}</p></article>)}</div></section>
 <section className="bv-section bv-tinted bv-split"><div><p className="bv-eyebrow">One code. Your own starting point.</p><h2>Scan here.<br /><em>Start discovering.</em></h2><p>A venue-specific QR code connects the sign to this page. The address stays stable while approved content changes.</p><p className="bv-small">Example code only. Confirm the published destination before printing signage.</p></div><div className="bv-qr">{origin?<><img src={`/api/wild/qr/${venue.id}`} alt={`QR code opening the ${venue.name} concept page`} width="256" height="256"/><a className="bv-button bv-green" download={`${venue.id}-concept-qr.svg`} href={`/api/wild/qr/${venue.id}?download=1`}>Download example QR</a><p className="bv-small">{origin}/wild/q/{venue.id}</p></>:<p>Your downloadable example QR will appear when the public site address is configured.</p>}</div></section>
