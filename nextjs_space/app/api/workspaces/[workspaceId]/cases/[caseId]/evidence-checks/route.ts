...
export async function GET(_request: Request, context: Context) {
  return privateRequest(async s => {
    ...
    return { evidenceChecks: await s.listEvidenceChecks(workspaceId, caseId) }
  })
}
export async function POST(request: Request, context: Context) {
  return privateRequest(async s => {
    const { workspaceId, caseId } = await context.params
    return { evidenceCheck: await s.createEvidenceCheck(workspaceId, caseId, await body(request)) }
  }, 201)
}
