...
export async function POST(request: Request) {
  ...
  try {
    ...
    await service.issue({email:input.email,password:input.password,ip,purpose:input.action==='admin-code'?'ADMIN_LOGIN':'VERIFY_EMAIL'})
    return Response.json({ok:true,message:'If the details match an eligible account, an email has been sent. Check your inbox and spam folder.'},{headers})
  } catch(error) {
