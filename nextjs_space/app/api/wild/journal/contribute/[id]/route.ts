...
export async function POST(request:Request, ctx:Ctx) {
 return journalRequest(async s=>{
  ...
  const raw=await body(request,4300000), metadata=contributionInput(raw), release=releaseInput(raw), input=photoInput(raw)
  await s.reserveScan(id)
  const clean=await preparePhoto(input)
  return s.contribute(id,{...clean,...metadata,...release})
 },false)
}
