...
export default [
  {
    ignores: ['node_modules/**', '.next/**', '.build/**', '.deploy/**', 'out/**', 'dist/**', 'build/**', 'public/parser/worker.mjs', 'prisma/**', 'scripts/**', '**/*.d.ts', 'next.config.js', 'postcss.config.js', 'tailwind.config.ts'],
  },
  {
    files: ['**/*.{ts,tsx}'],
    languageOptions: {
