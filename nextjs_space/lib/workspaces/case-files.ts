...
export function caseFiles(db: Database, actor: string, keyHex: string) {
  ...
  return {
    ...
    async export(w:string,c:string,render:(data:Record<string,unknown>)=>Promise<Buffer>,reportTitle?:string) {return db.transaction(async tx=>{
