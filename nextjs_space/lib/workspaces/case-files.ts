...
export function caseFiles(db: Database, actor: string, keyHex: string) {
  ...
  return {
    ...
    async export(w:string,c:string,render:(data:Record<string,unknown>)=>Promise<Buffer>,reportTitle?:string,appOrigin?:string) {return db.transaction(async tx=>{
      ...
      const id=randomUUID(); const manifest={version:1,rendererVersion:'case-renderer/2',exportId:id,workspaceId:w,caseId:c,appOrigin:appOrigin||undefined,case:caseRecord,reportTitle:reportTitle||undefined,createdAt:new Date().toISOString(),requestedBy:actor,filter:'ACCEPTED only; all dates, including unknown',events,excluded:allEvents.filter(x=>x.status!=='ACCEPTED').map(x=>({id:x.id,revision:x.revision,status:x.status})),notice:'Reviewed representation of sources, not verification of allegations or legal admissibility. Superseded records remain labelled. No originals attached.'};const bytes=await render(manifest);if(bytes.length>5*1024*1024)throw new WorkspaceError(413,'Export exceeds size limit');const hash=createHash('sha256').update(bytes).digest('hex')
