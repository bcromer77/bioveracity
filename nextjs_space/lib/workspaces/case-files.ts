...
export function caseFiles(db: Database, actor: string, keyHex: string) {
  ...
  return {
    ...
    async export(w:string,c:string,render:(data:Record<string,unknown>)=>Promise<Buffer>,reportTitle?:string,appOrigin?:string) {return db.transaction(async tx=>{
      await access(tx,w,c,'export');const [caseRecord]=await tx.query<{id:string;title:string;template:string}>('SELECT id,title,template FROM "PrivateCase" WHERE "workspaceId"=$1 AND id=$2 FOR UPDATE',[w,c]);const allEvents=await tx.query<EventRow>(eventQuery,[w,c]);const events=allEvents.filter(x=>x.status==='ACCEPTED')
      // BNG issued records carry the case's REVIEWED obligations alongside the source timeline.
      // Only reviewed obligations are exported (never DRAFT/UNRESOLVED). Non-BNG exports never
      // query obligations and never add the key, so their manifest bytes stay identical.
      const isBNG=caseRecord?.template==='BNG'
      const obligations=isBNG?await tx.query<Record<string,unknown>>(`SELECT o.id, o."createdAt", r.revision, r."sourceObligation", r."responsibleParty", r."dueDate", r."duePrecision", r.recurrence, r."expectedEvidence", r."evidenceStatus", r."reviewStatus", r."reviewedBy", r.note, r."createdAt" AS "revisedAt", p.locator AS "passageLocator", pd.name AS "passageDocumentName", d.name AS "documentName", ec.question AS "evidenceCheckQuestion", ec."reviewedScope" AS "evidenceCheckScope", ec."resultStatus" AS "evidenceCheckStatus", ec."resultSummary" AS "evidenceCheckSummary" FROM "PrivateCaseObligation" o LEFT JOIN "PrivateCasePassage" p ON p."workspaceId"=o."workspaceId" AND p."caseId"=o."caseId" AND p.id=o."passageId" LEFT JOIN "PrivateCaseDocument" pd ON pd.id=p."documentId" LEFT JOIN "PrivateCaseDocument" d ON d.id=o."documentId" LEFT JOIN "PrivateCaseEvidenceCheck" ec ON ec."workspaceId"=o."workspaceId" AND ec."caseId"=o."caseId" AND ec.id=o."evidenceCheckId" JOIN LATERAL (SELECT * FROM "PrivateCaseObligationRevision" rev WHERE rev."obligationId"=o.id ORDER BY rev.revision DESC LIMIT 1) r ON TRUE WHERE o."workspaceId"=$1 AND o."caseId"=$2 AND r."reviewStatus"='REVIEWED' ORDER BY r."dueDate" ASC NULLS LAST, o."createdAt" ASC, o.id`,[w,c]):[]
      if(!events.length && !(isBNG && obligations.length))throw new WorkspaceError(400,'Accept at least one source-linked event, or record at least one reviewed obligation, before exporting');if(events.length>400)throw new WorkspaceError(413,'Maximum 400 reviewed entries per export');if(obligations.length>400)throw new WorkspaceError(413,'Maximum 400 reviewed obligations per export')
      ...
      const id=randomUUID(); const manifest={version:1,rendererVersion:'case-renderer/2',exportId:id,workspaceId:w,caseId:c,appOrigin:appOrigin||undefined,case:caseRecord,reportTitle:reportTitle||undefined,createdAt:new Date().toISOString(),requestedBy:actor,filter:'ACCEPTED only; all dates, including unknown',events,excluded:allEvents.filter(x=>x.status!=='ACCEPTED').map(x=>({id:x.id,revision:x.revision,status:x.status})),...(isBNG && obligations.length ? { obligations, obligationFilter:'REVIEWED only; DRAFT and UNRESOLVED obligations are excluded' } : {}),notice:'Reviewed representation of sources, not verification of allegations or legal admissibility. Superseded records remain labelled. No originals attached.'};const bytes=await render(manifest);if(bytes.length>5*1024*1024)throw new WorkspaceError(413,'Export exceeds size limit');const hash=createHash('sha256').update(bytes).digest('hex')
