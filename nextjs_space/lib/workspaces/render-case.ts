...
export async function renderCase(manifest: Record<string, unknown>): Promise<Buffer> {
  ...
  pdf.setTitle(reportTitle||'BioVeracity reviewed case timeline');pdf.setProducer('BioVeracity case-renderer/1')
  ...
  if(reportTitle)line(reportTitle,18);line(String((manifest.case as {title:string}).title),16);line(`Issued ${manifest.createdAt}`);line(`Export ${manifest.exportId}`);line(String(manifest.notice));line('Unicode outside the PDF core font is shown as [U+codepoint]; the JSON manifest retains original Unicode.');y-=12
