...
export type CountyNatureResult = { county: string; status: 'ok' | 'partial' | 'unavailable' | 'unsupported'; checkedAt: string; records: CountyNatureRecord[]; inspected: number; excluded: number; note: string }
...
export async function fetchCountyNature(slug: string, options: ProviderOptions = {}): Promise<CountyNatureResult> {
 ...
 const result: CountyNatureResult = { county: county || slug, status: 'unsupported', checkedAt: (options.now?.() || new Date()).toISOString(), records: [], inspected: 0, excluded: 0, note: 'An API for this county is not connected. No substitute webpage summary is shown.' }
 ...
 try {
  ...
  for (const a of data.results.slice(0, 50)) {
   result.inspected++
   if (!a || !/^\d+$/.test(String(a.key)) || a.publishingOrgKey !== NBDC_PUBLISHER || a.countryCode !== 'IE' || a.gadm?.level1?.gid !== NBDC_GADM[county] || a.occurrenceStatus !== 'PRESENT' || a.informationWithheld || a.dataGeneralizations || typeof a.scientificName !== 'string' || !a.scientificName.trim() || typeof a.license !== 'string' || !/^https?:\/\/creativecommons\.org\/(?:publicdomain\/zero\/1\.0|licenses\/by\/4\.0)\/?$/.test(a.license)) {
  ...
  return { ...result, status: 'unavailable', records: [], note: 'The biodiversity API could not be checked. No webpage, invented record or previous success is substituted.' }
 }
}
