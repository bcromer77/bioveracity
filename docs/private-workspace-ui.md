...
Historical handoff for the original UI-only PR. The integrated branch now implements document imports, reviewed timelines and local PDF export; see [private-case-timeline.md](private-case-timeline.md) for current scope, checks and release blockers. The original dependency/typecheck limitations below have been resolved in the integrated build; hosted browser verification remains pending.
