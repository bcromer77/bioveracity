...
Assign a named accountable privacy operator and deputy; review `/admin/data-rights` daily. Optional daily reminders and response notices are available through `ops:daily` when `OPERATIONS_SEND_ENABLED=true`; the host must configure and monitor the schedule. Until then the queue has no automatic email alert. No automatic deletion worker or complete subject-access export worker is claimed.
