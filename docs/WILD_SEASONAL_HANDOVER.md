...
> Current release amendment: see WILD_MVP_RELEASE.md for the editorial-review migration, separate administrator approval, MVP storage decision and outstanding live release gates. Those instructions supersede owner-direct-publication descriptions below.
...
An authenticated venue owner can create a private draft, describe the venue, upload photographs, optionally import Google Trends, generate twelve monthly entries, edit/review them, and submit an immutable edition for a separate administrator to review and publish. The public hub selects that edition's current month in Europe/Dublin on each request. No background scheduler is needed. An expired or future plan does not display a misleading current monthly entry; the venue story remains available.

## What is automated, and what is still an owner decision

- Automated: draft persistence, twelve-month editorial generation, bounded county biodiversity API retrieval, optional historical Trends summarisation and suggested lead month, image scanning/resizing/metadata removal, review submission and approved publication snapshot, stable QR generation, calendar-based monthly feature selection.
- Owner decisions: accurate venue information, rights/representation attestation, upload selection, review of all twelve entries and submission for editorial review. An administrator reviews and publishes the exact submitted version. Saving new drafts never changes the public version. Publication is not business-identity verification or environmental certification.
...
- Wider-area records come from the bounded county API feed described in ONSITE_API_INTEGRATION.md. Unknown publication/event dates stay unknown. No regional species is asserted to occur at a venue. There is no new regulator endorsement, customer or partnership claim.
...
1. In an isolated staging database, apply `nextjs_space/prisma/migrations/20260914_wild_hubs/migration.sql` using the project's controlled migration process. Then apply `20260914_wild_editorial_review/migration.sql` to add the separate review queue. The first migration adds `WildHub`, `WildHubPhoto`, `WildHubPublication` and indexes/FKs to the existing User model; it does not seed businesses. Confirm Prisma migration history before deploying migrations; do not blindly replay the full migration directory against an existing database.
